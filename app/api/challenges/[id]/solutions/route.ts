...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (user && user.role !== "student") return Response.json({ error: "Please use a student account to submit a solution." }, { status: 403 });
  const db = getDb();
  ...
  const [proposal] = await db.insert(solutionProposals).values({ challengeId, teamName, institution, summary, approach, memberCount, submitterKey, userId: user?.id }).returning({ id: solutionProposals.id, status: solutionProposals.status });
