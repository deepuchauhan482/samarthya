...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [offer] = await db.insert(industryOffers).values({ challengeId, organization, supportType, message, submitterKey }).returning({ id: industryOffers.id, status: industryOffers.status });
